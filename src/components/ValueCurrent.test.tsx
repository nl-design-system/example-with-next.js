...
import { formatLabel, formatVisually, ValueCurrency } from "./ValueCurrency";
import "@testing-library/jest-dom";

describe("Currency value", () => {
  it("renders a data HTML element", () => {
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={123.45} />);
  ...
  it("renders a data HTML element with a value attribute", () => {
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={123} />);
  ...
  it("renders a BEM class name", () => {
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={1} />);

    const currency = container.querySelector(":only-child");

    expect(currency).toHaveClass("example-value");
    expect(currency).toHaveClass("example-value--currency");
  });

  it("renders rich text content", () => {
    const { container } = render(
      <ValueCurrency locale="nl-NL" currency="EUR" amount={123.45}>
  ...
  it("can be hidden", () => {
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={1} hidden />);
  ...
  it("can have a custom class name", () => {
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={1} className="reservation" />);
  ...
  it("supports ForwardRef in React", () => {
    ...
    const { container } = render(<ValueCurrency locale="nl-NL" currency="EUR" amount={1} ref={ref} />);
